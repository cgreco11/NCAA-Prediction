...
conferenceDf = pd.read_csv("data/TeamConferences.csv")
conferenceMapping = defaultdict(dict)
for idx, row in conferenceDf.iterrows():
    season = row["Season"]
    team = row["TeamID"]
    index = str(season) + "_" + str(team)
    conf = row["ConfAbbrev"]
    conferenceMapping[index] = conf

seasonAverages["Conference"] = seasonAverages.index.map(conferenceMapping)


poss_conferences = seasonAverages["Conference"].unique()
# Get one hot encoding of columns B
one_hot = pd.get_dummies(seasonAverages['Conference'])
# Drop column B as it is now encoded
seasonAverages = seasonAverages.drop('Conference',axis = 1)
# Join the encoded df
seasonAverages = seasonAverages.join(one_hot)
...
for idx, row in results_df.iterrows():
    ...
    deltaTop25Wins = team1_dict["Top 25 Wins"] - team2_dict["Top 25 Wins"]
    deltaTop10Wins = team1_dict["Top 10 Wins"] - team2_dict["Top 10 Wins"]
    out_dict = {
        ...
        'deltaTop25' : round(deltaTop25Wins, 2),
        'deltaTop10' : round(deltaTop10Wins, 2)
    }

    for i in poss_conferences:
        winning_col = "w_" + i
        losing_col = "l_" + i
        out_dict[winning_col] = team1_dict[i]
        out_dict[losing_col] = team2_dict[i]
